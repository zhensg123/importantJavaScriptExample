<template>
  <div>
    <template v-for="tag in tags">
      <el-tag :key="tag.key" :type="tag.type" :color="tag.color" :size="tag.size" :effect="tag.effect">{{tag.label}}</el-tag>
    </template>
  </div>
</template>

<script>
  import {isFunction, isArray} from '@/util/types'
  ...
  */
  export default {
    ...
    methods: {
      update() {
        this.tags = []
        let data = getObjValue(this.row, this.column.prop)
        if (data === null) return
        let format = this.column.config.format
        let formatData = (data, key) => {
